...
class LSTM:
	...
	def _build_model_hahahaha(self):
		# self.x = tf.placeholder(tf.float32,[self.batch_size, None , self.num_notes, 1])
		self.x = tf.placeholder(tf.float32,[self.batch_size, None , self.num_notes, 1])
		self.y_truth = tf.placeholder(tf.float32,[self.batch_size, None , self.num_notes, 1])
		...
		self.deconv1 = tf.layers.conv2d_transpose(pool3,
			filters=128,
			kernel_size=[5,5],
			padding="same",
			strides=[self.stride_length, self.stride_length],
		...
		self.losses = tf.losses.mean_squared_error(self.final_outputs, self.y_truth)

		self.total_loss = tf.reduce_mean(self.losses)
		self.optimizer = tf.train.AdamOptimizer(tf.constant(0.005),0.995)
		...
		self.variables2 = tf.trainable_variables()
	...
	def train(self, x):
		...
		while True:
			...
			# loss, losses, prediction, truth, before, after, gradients = self.sess.run([self.total_loss, self.losses, self.logits, self.y_truth, self.variables, self.variables2, self.gradients], feed_dict={self.x: x_mat, self.y_truth: y_mat})
			loss, _, losses = self.sess.run([self.total_loss, self.optimizer, self.losses], feed_dict={self.x: x_mat, self.y_truth: y_mat})
			pdb.set_trace()
			counter +=1
			if counter % 1 == 0:
				# print(grad)
				...
				# else:
				# 	print("Wow different!")
				# if np.array_equal(last_variables, before):
				# 	print("Same variables")
				# else:
				# 	print("Dayum son they changed")
				# last_prediction = prediction
				# last_variables = before
