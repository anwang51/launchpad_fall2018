...
class vae_mnist():
    ...
    def build_model(self):
        # with tf.device('/gpu:0'):
            self.images = tf.placeholder(tf.float32, [None, self.num_timesteps, self.num_notes])
    ...
    def encoder(self, x):
        ...
        # w0 = tf.get_variable('w0', [28, self.n_hidden], initializer=w_init
            ...
            w2 = tf.Variable(tf.random_normal((h1.get_shape().as_list()[1], self.n_hidden), stddev=0.01), dtype=tf.float32)
            b2 = tf.Variable(tf.random_normal((self.n_hidden,), stddev=0.01), dtype=tf.float32)
            h2 = tf.matmul(h1, w2) + b2
            h2 = tf.nn.leaky_relu(h2)

            w_out = tf.Variable(tf.random_normal((h2.get_shape().as_list()[1], self.n_z * 2), stddev=0.01), dtype=tf.float32)
            b_out = tf.Variable(tf.random_normal((self.n_z * 2,), stddev=0.01), dtype=tf.float32)
            params = tf.matmul(h3, w_out) + b_out
    ...
    def decoder(self, z):
        # with tf.device('/gpu:0'):
            ...
            # w3 = tf.Variable(tf.random_normal((h2.get_shape().as_list()[1], self.n_hidden), stddev=0.01), dtype=tf.float32)
            # b3 = tf.Variable(tf.random_normal((self.n_hidden,), stddev=0.01), dtype=tf.float32)
            # h3 = tf.matmul(h2, w3) + b3
            # h3 = tf.nn.leaky_relu(h3)

            w_out = tf.Variable(tf.random_normal((h2.get_shape().as_list()[1], self.input_size), stddev=0.01), dtype=tf.float32)
            b_out = tf.Variable(tf.random_normal((self.input_size,), stddev=0.01), dtype=tf.float32)
            y = tf.sigmoid(tf.matmul(h3, w_out) + b_out)
    ...
    def train(self, xmat):
        ...
        while True:
            ...
            for i in range(total_batch):
                ...
                batch_xs_input = np.array(next(x_iter))
...
test, train = data_io.test_train_sets_lpd5(root_dir + "data/lpd_5/", track_name='Piano', split_len=LEN_SONG)
